...
import json
from ai_providers import ClipAnalysisError, analyze_clips, config_from_env
...
def get_viral_clips(transcript_result, video_duration):
    ...
    print(f"🤖  Analyzing with {config.provider} model: {config.model or config.azure_deployment}")

    try:
        result_json = analyze_clips(transcript_result, video_duration, config)
        usage = result_json.get("cost_analysis", {}).get("usage") or {}
        if usage:
            print(f"💰 Token Usage ({config.provider}/{config.model}): {usage}")
        return result_json
